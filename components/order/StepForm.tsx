...
const StepForm: React.FC<StepFormProps> = ({ order, workflowStep, onStepDataChange }) => {
            ...
            case 'checkbox':
                return (
                    <div className="flex items-center pt-6">
                        <input id={field.id} type="checkbox" name={field.name} checked={!!value} onChange={handleChange} className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                        <label htmlFor={field.id} className="mr-2 block text-sm font-medium text-gray-700">{field.label}</label>
                    </div>
                );
            case 'select':
                return (
                    <select name={field.name} value={String(value || '')} onChange={handleChange} className={`w-full border rounded-lg shadow-sm p-3 bg-white appearance-none focus:ring-2 focus:ring-blue-500 ${error ? 'border-red-500' : 'border-gray-300'}`}>
                        <option value="">انتخاب کنید...</option>
                        {(field.options || []).map((opt, i) => (<option key={i} value={opt.trim()}>{opt.trim()}</option>))}
                    </select>
                );
            case 'product':
                return <ProductSelectorField field={field} value={Array.isArray(value) ? value : []} onChange={handleChange} error={!!error} />;
            case 'textarea':
                return <textarea name={field.name} value={String(value || '')} onChange={handleChange} rows={4} className={`w-full border rounded-lg shadow-sm p-3 focus:ring-2 focus:ring-blue-500 ${error ? 'border-red-500' : 'border-gray-300'}`} />;
            default:
                return <input type={field.type} name={field.name} value={String(value || '')} onChange={handleChange} className={`w-full border rounded-lg shadow-sm p-3 focus:ring-2 focus:ring-blue-500 ${error ? 'border-red-500' : 'border-gray-300'}`} />;
        }
    };
